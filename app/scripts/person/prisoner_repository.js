(function() {
    ...
    */
    ...
    .service('prisonerRepository', function($q, _, AdvancedSparqlService, objectMapperService,
                dateUtilService, ENDPOINT_CONFIG) {
        var endpoint = new AdvancedSparqlService(ENDPOINT_CONFIG, objectMapperService);
        ...
        this.getPersonPrisonerRecord = function(id, lang) {
            ...
            prisonerRecordQry += 'SELECT DISTINCT ?id ?prefLabel__id ?prefLabel__propertyLabel ';
            ...
            '   BIND("Nimi" AS ?prefLabel__lbl ) ' +
            '   OPTIONAL { ?rei rdf:subject ?id . } ';

            prisonerRecordProperties.forEach(function(prop) {
                prisonerRecordQry += generatePrisonerPropertyQry(prop);
            });

            prisonerRecordQry += '} ';
            var qry = prisonerRecordQry.format(id);
        ...
        function generatePrisonerPropertyQry(property) {
            ...
            var qry =
            ' OPTIONAL { ' +
            '  ?id prisoners:<PROPERTY> ?<PROPERTY>__id . ' +
            //'  OPTIONAL { ?<PROPERTY>__id skos:prefLabel ?<PROPERTY> . } ' +
            '  prisoners:<PROPERTY> skos:prefLabel ?<PROPERTY>__propertyLabel . ' +
            '  FILTER(lang(?<PROPERTY>__propertyLabel)="fi") . ' +
            '  ?rei rdf:predicate prisoners:<PROPERTY> . ' +
            '  ?rei rdf:object ?<PROPERTY>__id . ' +
            '  ?rei dc:source ?<PROPERTY>__source . ' +
            ' } ' +
            ' OPTIONAL { ' +
            '  ?id prisoners:<PROPERTY> ?<PROPERTY>__id . ' +
        ...
        function generatePrisonerSelectRow(property) {
            return '?' + property + '__id ?' + property + '__propertyLabel ?' + property + '__source ';
        }
